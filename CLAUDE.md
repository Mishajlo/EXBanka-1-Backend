...
```
After both stacks are healthy, register each as a peer in the other via `POST /api/v3/peer-banks`. `base_url` is the FULL SI-TX path prefix that the peer exposes its `/interbank`, `/public-stock`, and `/negotiations` endpoints under — for this codebase that's **`http://host.docker.internal:<gateway-port>/api/v3/cross-bank-protocol`** (the `/cross-bank-protocol` segment is REQUIRED — the legacy `/api/v3/interbank` aliases were removed on 2026-05-29; registering a bare `/api/v3` base_url makes outbound NEW_TX hit a 404 "route not found" and the transaction hangs in `prepared`). The outbound egress client (`interbank-service/internal/handler/peer_egress_grpc_handler.go` via `interbank-service/internal/sitx/peer_http_client.go`) only appends the leaf names (`/interbank`, `/public-stock`, `/negotiations`, `/user`) so cohort banks with different gateway path layouts all interop — each peer's prefix lives in its registration row, not in our client code. Peer auth: send a shared `api_token` (X-Api-Key) on each side (HMAC keys optional); the same token on both registrations is sufficient. Requires docker compose v2.24+ for the `!override` tag.
