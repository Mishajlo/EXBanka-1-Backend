...
- ⏳ **Next: transaction-service cluster** — Task 8 (executor consumes account/asset type tags) → Task 9 (NO-vote posting index) → Task 10 (metadata cols + ledger memo) → **Task 11 (transactionId correlation + per-message idem — 2nd high-risk pivot)** → Task 12 (outbound builds spec postings + 202 handling). This module stays RED until ~Task 12 lands. Then Task 14 (`/user` display names + OWN_BANK_NAME config + compose), Task 15 (OTC money-as-number), Task 16 (stock-service accept postings), Task 17 (fixtures), Task 18 (integration), Task 19 (docs).
